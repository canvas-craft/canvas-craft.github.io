...
const conversation = {
    ...
    state: '*Shape State* The asterisk signifies that the shape has invalid dimensions or coordinates',
    ...
    shapePos: '*Shape Pos* Access the position of the current shape with the object <span class = code><span style="color:var(--operator)">{</span><span style="color:var(--variable)">x</span>, <span style="color:var(--variable)">y</span><span style="color:var(--operator)">}</span></span>',
    splitLayer: '*Split Layer* Split the current layer in two. You can select the point at which it does this'
}
...
const queries = [
    ...
    ['whatdoesthesavedefaultcolourcolordohowdoesworknexttoiconlogosymbol', '<h1>What does the colour icon do?</h1><p>The deault colour button is found in the shape panel, and has been added for the purpose of creating multiple shapes with the same colour.</p><p>By default, all shapes are created with a <b>random colour</b>, so this feature allows you to control it. If you would like to switch back to <b>random colour mode</b> or change the default colour to something else, head to the settings tab under the "Shapes" subheading.</p>'],
    ['ticksymboltemplateiconlogocheck', '<h1>What does the check icon next to the template selector do?</h1><p>The "check" icon <button class = reallyTiny><img src = src/default.svg></button> next to the template selector allows you to save the template as default. A default template means that any shape you create will automatically have the template to begin with.</p>'],
    ['whatdoesthesavethiscodeastemplatemeandohowdoeswork', '<h1>How does the "Save this code as template" button work?</h1><p>The "Save this code as template" button is used to store <b>code textures for later.</b> For example, if you have created a texture that you think might be useful for other shapes, you can use the feature to <b>add it to the list of templates.</b> A template, as exlained in the query "What are code templates?", is a code texture that can be accessed by any shape in your image. Once you\'ve saved your texture, you can assign it to any shape in the image by pressing the "Current Template" button.</p>'],
    ['helpassistantbot', '<h1>What is the help assistant?</h1><p>The help assistant - found by clicking the question mark at the top-left of the screen - is a basic way to find out what each part of CanvasCraft does. Simply hover over something and a description of it will appear in the help box.</p>'],
    ['whatarelayerscollapsedrophidenew', '<h1>What is a layer?</h1><p>A layer is a container that stores shapes. The "New Layer" button allows you to create a layer beneath the current one. The "Drop" button lets you swap the positions of the layer with the one beneath it. The "Collapse" button lets you shrink the layer box. This makes navigating through multiple layers easier, as you don\'t have to scroll through the contents of each one.</p>']
]
...
querySearch.oninput = () => {
    ...
    for (let i = 0; i < queries.length; i ++) {
        ...
        for (let j = 0; j < queryWords.length; j ++) {
            ...
            if (name.includes(word)) dic.score += word.length * 2
